...
import { Ban, LogOut, Plus } from 'lucide-vue-next';
import {
...
const {
  ...
  searching,
  clear: clearSearch,
} = useUserSearch({ filter: (u) => !activeEmails.value.has(u.email) });
...
<template>
  <BaseOverlay
    ...
    :padded="false"
    title="Members"
    :description="`${active.length} in this group`"
    @close="emit('close')"
  >
    <div class="px-6">
    ...
    <div class="mt-4 min-h-0 flex-1 overflow-y-auto px-6">
      ...
      <template v-if="former.length">
        <p class="mt-4 mb-1 text-xs font-medium text-muted-foreground">Previously in this group</p>
